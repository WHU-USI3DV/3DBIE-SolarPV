...
from core.solver import adjust_learning_rate, adjust_learning_rate_use_stair
...
def train(cfg, local_rank, distributed):
    ...
    optimizer_conv_down = torch.optim.SGD(conv_down.parameters(), lr=cfg.SOLVER.BASE_LR * 10,
                                          momentum=cfg.SOLVER.MOMENTUM, weight_decay=cfg.SOLVER.WEIGHT_DECAY)
    optimizer_conv_down.zero_grad()

    optimizer_SE = torch.optim.SGD(SE.parameters(), lr=cfg.SOLVER.BASE_LR * 10,
                                          momentum=cfg.SOLVER.MOMENTUM, weight_decay=cfg.SOLVER.WEIGHT_DECAY)
    ...
    if cfg.resume:
        ...
        if "ConvDown" in checkpoint:
            conv_down_weights = checkpoint['ConvDown'] if distributed else strip_prefix_if_present(
                checkpoint['ConvDown'], 'module.')
    ...
    # Freeze parameters in feature_extractor, classifier
    ...
    for i, ((src_input, src_label, src_name), (tgt_input, _, _)) in enumerate(zip(src_train_loader, tgt_train_loader)):
        ...
        if (iteration == cfg.SOLVER.MAX_ITER or iteration % cfg.SOLVER.CHECKPOINT_PERIOD == 0) and save_to_disk:
            ...
            torch.save({'iteration': iteration, 'SE': SE.state_dict(), 'ConvDown': conv_down.state_dict(),
