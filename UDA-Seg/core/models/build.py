...
def build_feature_extractor(cfg, multi_scale=False):
    model_name, backbone_name = cfg.MODEL.NAME.split('_')
    if backbone_name.startswith('resnet'):
        backbone = resnet_feature_extractor(backbone_name, pretrained_weights=cfg.MODEL.WEIGHTS, aux=False, pretrained_backbone=True, freeze_bn=cfg.MODEL.FREEZE_BN, multi_scale=multi_scale)
...
def build_classifier(cfg, level):
...
def build_binary_discriminator(cfg, num_features=None, mid_nc=256):
    _, backbone_name = cfg.MODEL.NAME.split('_')
    if backbone_name.startswith('vgg'):
        if num_features is None:
            num_features = 1024
        model_D = FCDiscriminator(num_features, mid_nc)
    elif backbone_name.startswith('resnet'):
        if num_features is None:
            num_features = 2048
        model_D = FCDiscriminator(num_features, mid_nc)
