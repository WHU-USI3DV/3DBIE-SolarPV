...
class resnet_feature_extractor(nn.Module):
    def __init__(self, backbone_name, pretrained_weights=None, aux=False, pretrained_backbone=True, freeze_bn=False, multi_scale=False):
        ...
        backbone = resnet.__dict__[backbone_name](
            ...
            norm_layer=bn_layer)
        if self.multi_scale:            
            return_layers = {'layer4': 'out', 'layer3': 'low_fea'}
        else:
    ...
    def forward(self, x):
        if self.multi_scale:
            ...
            feas = self.backbone(x)
            low_fea = feas['low_fea']
            out = feas['out']

            ms_fea = torch.cat([out, low_fea], 1)
            return out, ms_fea
        else:
