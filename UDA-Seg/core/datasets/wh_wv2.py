...
class wh_wv2_Dataset(data.Dataset):
    def __init__(
            self,
            data_root,
            data_list,
        ...
        for fname in content:
            ...
            self.data_list.append(
                {
                    "img": os.path.join(
                        self.data_root, "img/%s" % name
                    ),
                    "label": os.path.join(
                        self.data_root,
                        "labels/%s" % ("buildings" + name.split("wh_wv2")[1].replace('.tif', '.npy'))
                    ),
                    "name": name,
                }
            )
    ...
    def __getitem__(self, index):
        ...
        image = io.imread(datafiles["img"])[:, :, 0:3].astype(np.float32)
        # image = io.imread(datafiles["img"]).transpose(2, 0, 1)
        # image = Image.open(datafiles["img"]).convert('RGB')
        label = np.ones([650, 650, 3])
